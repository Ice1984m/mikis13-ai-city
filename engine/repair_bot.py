...
def check_file(path):
    p = ROOT / path
    return {"name": path, "ok": p.exists() and p.stat().st_size > 0}
...
def main():
    ...
    json_org = run([
        "python", "-m", "json.tool", "config/organization.json",
    ])
    checks.append({
    ...
    security = run(["bash", "scripts/security.sh"])
    checks.append({
        "name": "security-gate",
        "ok": security["code"] == 0,
    })
    ...
    health = round(100 * (len(checks) - len(failed)) / max(1, len(checks)))
    ...
    result = {
        ...
        "status": "HEALTHY" if not failed else "REPAIR_REQUIRED",
    }

    out = STATE / "repair" / "latest.json"
    ...
    if failed:
        report += ["", "## Required repair", ""]
        for x in failed:
            report.append(f"- repair `{x}`")

    (REPORTS / "repair-latest.md").write_text("\n".join(report) + "\n")
