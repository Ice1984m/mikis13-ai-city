...
echo; echo "1/6 REPAIR BOT"
python engine/repair_bot.py

echo; echo "2/6 POLICY TESTS"
python tests/test_city.py

echo; echo "3/6 COUNCIL"
python engine/city.py

echo; echo "4/6 LEARNING"
python engine/prompt_lab.py

echo; echo "5/6 INTELLIGENCE METER"
python engine/intelligence_meter.py

echo; echo "6/6 FINAL REPAIR CHECK"
python engine/repair_bot.py

ls -1t "$ROOT/logs"/evolve-*.log 2>/dev/null \
  | tail -n +31 \
  | xargs -r rm -f
