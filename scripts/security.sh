...
python -m json.tool config/organization.json >/dev/null
...
HITS="$(mktemp "$TMP_BASE/mikis-city-secret-hits.XXXXXX")"
trap 'rm -f "$HITS"' EXIT
...
if git grep -En "$PATTERN" -- \
  ':!docs/' ':!state/' ':!reports/' ':!logs/' \
  >"$HITS" 2>/dev/null
then
  echo "❌ Mogelijk geheim gevonden"
  sed -E 's/[A-Za-z0-9_-]{12,}/[REDACTED]/g' "$HITS"
  exit 1
fi
